...
void LinearProbingAggregateHashTable<V>::add_batch(int *input_keys, V *input_values, int len)
{
  ...
  int inv[SIMD_WIDTH];
  int off[SIMD_WIDTH];
  std::fill(inv, inv + SIMD_WIDTH, -1);
  std::fill(off, off + SIMD_WIDTH, 0);
  ...
  // }
  for (; i + SIMD_WIDTH <= len;) {
    selective_load(input_keys, i, key, *(__m256i*)&inv);
    selective_load(input_values, i, value, *(__m256i*)&inv);
    // selective_load(input_values, 0, value, *(__m256i*)&inv);
    i += -mm256_sum_epi32(inv, SIMD_WIDTH);
    __m256i hash = AddOperator::operation(_mm256_loadu_si256((__m256i*)key),
        _mm256_loadu_si256((__m256i*)off));
    int hash_list[SIMD_WIDTH];
    _mm256_storeu_si256((__m256i*)hash_list, hash);
    for (int j = 0; j < SIMD_WIDTH; j++) {
      int hv = hash_list[j] % capacity_;
      if (keys_[hv] == EMPTY_KEY) {
        keys_[hv] = key[j];
        size_++;
      }
      if (keys_[hv] == key[j]) {
        aggregate(&values_[hv], value[j]);
        inv[j] = -1;
        off[j] = 0;
      } else {
        inv[j] = 0;
  ...
  //7. 通过标量线性探测，处理剩余键值对
  for (; i < len; i++) {
    int hv = input_keys[i] % capacity_;
    while (keys_[hv] != EMPTY_KEY && keys_[hv] != input_keys[i]) {
      hv = (hv + 1) % capacity_;
    }
    if (keys_[hv] == EMPTY_KEY) {
      keys_[hv] = input_keys[i];
      size_++;
    }
    aggregate(&values_[hv], input_values[i]);
  }
  for (int j = 0; j < SIMD_WIDTH; j++) {
    if (inv[j] == -1) {
      continue;
    }
    int hv = key[j] % capacity_;
    while (keys_[hv] != EMPTY_KEY && keys_[hv] != key[j]) {
      hv = (hv + 1) % capacity_;
    }
    if (keys_[hv] == EMPTY_KEY) {
      keys_[hv] = key[j];
